...
/**
 * 
 * @author Administrator 2017-8-13 2. 第二种解法，是利用CyclicBarrierAPI；
 *         CyclicBarrier可以实现让一组线程在全部到达Barrier时(执行await())，再一起同时执行，
 *         并且所有线程释放后，还能复用它,即为Cyclic。
 *         CyclicBarrier类提供两个构造器：
 */
public class MethodFour_CyclicBarrier {

	private final CyclicBarrier barrier;
	private final List<String> list;

	public MethodFour_CyclicBarrier() {
...
//				System.out.println(Arrays.toString(list.toArray()));
	...
	public static void main(String args[]) {
		MethodFour_CyclicBarrier four = new MethodFour_CyclicBarrier();
